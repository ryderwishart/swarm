...
def setup_groq_client() -> Groq:
    """Set up and return a Groq client instance."""
...
def batch_translations(translations: List[Dict], scenario: TranslationScenario, qa_bot: Groq, swarm_client: Groq) -> str:
...
def setup_agents(scenario: TranslationScenario) -> tuple:
    ...
    # Create a single Groq client instance
    groq_client = setup_groq_client()
    
    # Return the same client for all roles
    return (groq_client,) * 5  # Returns 5 references to the same client

def call_llm(client: Groq, prompt: str, expect_json: bool = False) -> str:
    """Make a single call to the Groq LLM with retries and parsing."""
    max_retries = 3
    for attempt in range(max_retries):
        try:
            response = client.chat.completions.create(
                model="mixtral-8x7b-32768",  # Groq's model with larger context window
                messages=[{
                    "role": "user", 
...
def analyze_text(client: Groq, text: str, source_label: str, target_label: str, book_context: str = "") -> Dict:
...
def translate_components(client: Groq, analysis: Dict, source_label: str, target_label: str, 
...
def combine_translation(client: Groq, original: str, translations: Dict, 
...
def consolidate_translation(client: Groq, final_translation: str, original: str,
...
def translate_with_llm(text: str, scenario: TranslationScenario, 
                      client: Groq, translation_memory: TranslationMemory,
